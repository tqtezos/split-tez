...
module Lorentz.Contracts.SplitTezRef where
...
splitTezRefContract :: Natural -> Address -> ContractCode Parameter Storage
splitTezRefContract totalShares delegateAdmin = do
  unpair
  caseT @Parameter
    ( #cSetDelegate /-> do
      sender
    ...
    , #cSplit /-> do
      caseT @SplitParam
        ...
        , #cFlush /-> do
          ...
          unStorage
          unpair
          stackType @(TotalBalance & Ledger & [(ToFlush, UserRef)] & '[])
          dup
          dip
            (do
              push totalShares
...
printSplitTezRefContract :: Natural -> Address -> Maybe FilePath -> Bool -> IO ()
printSplitTezRefContract totalShares delegateAdmin mOutput forceOneLine' =
  maybe TL.putStrLn Utf8.writeFile mOutput $
  printLorentzContract forceOneLine' $
    (defaultContract (splitTezRefContract totalShares delegateAdmin))
      { cDisableInitialCast = True }

printExampleSplitTezRef :: IO ()
printExampleSplitTezRef =
  printSplitTezRefContract
  0
  (read "tz1QS8VYYVDjv7iReBzXeheL6x63A1oATTj8")
  (Just "contracts/split_tez_ref.tz")
  False
